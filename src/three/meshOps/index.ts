...
export function generateFacialMorphs(
  ...
  */
  ...
  // Apple Cheek Tips
  let appleCheekTipL = new THREE.Vector3();
  ...
  const visualizerByJawHeightMorph: THREE.Vector3[] = [];
  const visualizerByJawDepthMorph: THREE.Vector3[] = [];
  ...
  /**
   * Ⅰ.Ⅹ APPLE CHEEK TIPS DETECTION
   */
  // Detect the apple cheek tips based on the center vector of the eye nodes
  const appleCheekTipOffsetX = 2;
  appleCheekTipL.x = centerEyeRNode.x - appleCheekTipOffsetX;
  appleCheekTipR.x = centerEyeLNode.x + appleCheekTipOffsetX;
  appleCheekTipL.y = appleCheekTipR.y = noseTip.y + 0.5;
  appleCheekTipL.z = appleCheekTipR.z = centerEyeLNode.z + 1.3;
  appleCheekTipLM.copy(appleCheekTipL);
  appleCheekTipRM.copy(appleCheekTipR);
  appleCheekTipLM.x += appleCheekTipOffsetX * 0.5;
  appleCheekTipRM.x -= appleCheekTipOffsetX * 0.5;
  appleCheekTipLM.z += 1.3;
  appleCheekTipRM.z += 1.3;

  console.log(
    "\n -- generateFacialMorphs -- appleCheekTipL calculated ->",
    appleCheekTipL.z === -Infinity ? "Not Found" : appleCheekTipL,
  );
  console.log(
    "\n -- generateFacialMorphs -- appleCheekTipR calculated ->",
    appleCheekTipR.z === -Infinity ? "Not Found" : appleCheekTipR,
  );
  ...
  const jawHeightTarget = new Float32Array(positions.count * 3);
  const jawDepthTarget = new Float32Array(positions.count * 3);
  ...
  for (let i = 0; i < positions.count; i++) {
    ...
    // --- K.0 GENERATE JAW MORPH (Widening) ---
    applyMorph(
      vertex,
      i,
      jawTipL,
    ...
    applyMorph(
      ...
      visualizerByJawHeightMorph,
      1.8,
    );

    // --- K.2 GENERATE JAW MORPH (Depth) ---
    applyMorph(
      vertex,
      i,
      jawTipL,
      ...
      visualizerByJawDepthMorph,
      4,
    );

    // --- L.0 GENERATE JAW SIDES WIDTH MORPH (Widening) ---
    applyMorph(
      vertex,
      i,
      jawTipL,
    ...
    // --- L.1 GENERATE JAW SIDES HEIGHT MORPH (Height) ---
    applyMorph(
      vertex,
      i,
      jawTipL,
    ...
    // --- L.2 GENERATE JAW SIDES DEPTH MORPH (Depth) ---
    applyMorph(
      vertex,
      i,
      jawTipL,
    ...
    // --- Q.0 GENERATE APPLE CHEEK WIDTH MORPH (widening) ---
    applyMorph(
      vertex,
      i,
      appleCheekTipL,
      null,
      appleCheekTipR,
      { xRange: 1.4, yRange: 1.4, zRange: 3 },
      appleCheekWidthTarget,
      "widening",
      {
        widening: {
          isCurveInverted: true,
    ...
    // --- Q.1 GENERATE APPLE CHEEK HEIGHT MORPH (height) ---
    applyMorph(
      vertex,
      i,
      appleCheekTipLM,
    ...
    // --- Q.2 GENERATE APPLE CHEEK DEPTH MORPH (depth) ---
    applyMorph(
      vertex,
      i,
      appleCheekTipLM,
  ...
  const jawHeightAttr = new THREE.BufferAttribute(jawHeightTarget, 3);
  const jawDepthAttr = new THREE.BufferAttribute(jawDepthTarget, 3);
  const jawSidesWidthAttr = new THREE.BufferAttribute(jawSidesWidthTarget, 3);
  const jawSidesHeightAttr = new THREE.BufferAttribute(jawSidesHeightTarget, 3);
  const jawSidesDepthAttr = new THREE.BufferAttribute(jawSidesDepthTarget, 3);
  ...
  const appleCheekWidthAttr = new THREE.BufferAttribute(
    appleCheekWidthTarget,
    3,
  );
  const appleCheekHeightAttr = new THREE.BufferAttribute(
    appleCheekHeightTarget,
    3,
  );
  const appleCheekDepthAttr = new THREE.BufferAttribute(
    appleCheekDepthTarget,
    3,
  );
  ...
  jawHeightAttr.name = "jawHeight";
  jawDepthAttr.name = "jawDepth";
  ...
  return {
    ...
    // Apple Cheek Tips
    visualizerAppleCheekTipL: appleCheekTipL,
...
export function generateFacialMorphsTst(
  ...
  */
  ...
  return {
    ...
    visualizerAppleCheekTipL: new THREE.Vector3(),
...
function applyMorph(
  ...
  if (dx < xRange && dy < yRange && dz < zRange) {
    ...
    if (isHeight || isDepth) {
      ...
      */
      if (isHeight)
        targetArray[index * 3 + 1] += isApplyModeAddition
          ? totalInfluence
          : -totalInfluence;

      // if (isDepth) targetArray[index * 3 + 2] -= totalInfluence;
      if (isDepth)
        targetArray[index * 3 + 2] += isApplyModeAddition
          ? totalInfluence
          : -totalInfluence;
