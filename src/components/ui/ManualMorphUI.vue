    ...
    <Transition name="slide-up">
      ...
      >
        <div class="flex flex-col gap-2 items-center mb-2">
          <h3 class="text-cyan-400 font-futuristic tracking-widest text-sm uppercase">
            Manual Morph Selection
          </h3>
          <div class="h-px w-32 bg-linear-to-r from-transparent via-cyan-500/50 to-transparent"></div>
        </div>

        <div class="flex gap-3">
          <Button
            @click="setStage('jaw')"
            :class="{ 'border-2 border-cyan-400 shadow-[0_0_10px_rgba(34,211,238,0.5)]': manualMorphSelectionStage === 'jaw' }"
          >
            {{ manualJawTipL ? '✅ Jaw' : 'Select Jaw' }}
          </Button>
          <Button
            @click="setStage('eyeBrow')"
            :class="{ 'border-2 border-cyan-400 shadow-[0_0_10px_rgba(34,211,238,0.5)]': manualMorphSelectionStage === 'eyeBrow' }"
          >
            {{ manualEyeBrowTipL ? '✅ Eyebrow' : 'Select Eyebrow' }}
          </Button>
          <Button
            @click="setStage('mouseCornersWidth')"
            :class="{ 'border-2 border-cyan-400 shadow-[0_0_10px_rgba(34,211,238,0.5)]': manualMorphSelectionStage === 'mouseCornersWidth' }"
          >
            {{ manualMouseCornerTipL ? '✅ Mouse' : 'Select Mouse' }}
          </Button>
        </div>
        ...
        <div class="flex gap-4 mt-2">
          <button
            @click="cancel"
            class="px-6 py-2 border border-cyan-500/20 text-cyan-500/50 hover:border-cyan-500/50 hover:text-cyan-400 transition-all text-xs uppercase tracking-widest font-futuristic"
          >
            Cancel
          </button>
...
const startManualMorph = () => {
  modelsStore.setManualMorphGenerationMode(true);
};
