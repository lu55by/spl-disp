...
- **Store Integration**: Connected the drop event to `modelsStore.imoprtObjWithModelHeight` to handle the actual file loading.

## How it works

1. **Drag**: When you drag a file into the window, a futuristic overlay appears with a glowing ring and a centered plus icon.
2. **Drop**: When you release the file, the overlay disappears and the file is processed by `modelsStore.imoprtObjWithModelHeight`.
